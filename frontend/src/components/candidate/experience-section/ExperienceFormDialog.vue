    ...
    <div class="flex flex-column gap-3 pt-2">
      ...
      <div class="flex gap-2">
        <div class="flex flex-column gap-2 w-6">
          <label>From</label>
          <InputText v-model="localForm.startDate" class="w-full" placeholder="Ex: 2020" />
        </div>
        <div class="flex flex-column gap-2 w-6">
          <label>Until</label>
          <InputText v-model="localForm.endDate" class="w-full" placeholder="Ex: Present" />
        </div>
      </div>
      <div class="flex flex-column gap-2">
        <label>Description</label>
        <Textarea v-model="localForm.description" rows="3" class="w-full" autoResize />
      </div>
    </div>
    <template #footer>
      <Button label="Cancel" text severity="secondary" @click="$emit('update:visible', false)" />
      <Button label="Save" @click="save" :disabled="!localForm.title || !localForm.company" />
    </template>
  </Dialog>
</template>
