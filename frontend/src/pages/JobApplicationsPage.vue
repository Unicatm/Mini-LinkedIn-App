  ...
  <div class="surface-ground min-h-screen pb-6 pt-5">
    <div class="max-w-7xl mx-auto px-4">
      ...
      >
        <div>
          ...
          <div class="text-500 text-sm mt-1">
            ...
            <i class="pi pi-wallet mr-1"></i> {{ jobDetails.salary || "Confidential" }}
          </div>
        </div>
      </div>
      ...
      <div v-else class="surface-card p-4 border-round-xl shadow-1">
        <DataTable :value="applications" responsiveLayout="scroll" paginator :rows="10">
          <Column header="Candidate">
            <template #body="{ data }">
              <div class="flex align-items-center gap-2">
                <Avatar icon="pi pi-user" shape="circle" class="bg-blue-100 text-blue-600" />
                <div class="flex flex-column">
                  <span class="font-medium text-900">{{ data.candidateSnapshot?.name }}</span>
                  <span class="text-sm text-500">{{ data.candidateSnapshot?.email }}</span>
                </div>
              </div>
            </template>
          </Column>
...
const applications = computed(() => jobStore.currentJobApplications);

onMounted(async () => {
  const jobId = route.params.id;

  jobDetails.value = await jobStore.fetchJobById(jobId);
  await jobStore.fetchApplicationsForJob(jobId);
});
